 ...
 */
    ...
    #define BINHUB_PRINT_ENV_VAR(var) do { \
        ...
        if(var == (void*)0) { \
            binhub_cprintf((binhub_tcolor_t){CLR_RED, CLR_NONE}, "The environment variable " #var " does not exist!\n"); \
        } else { \
            binhub_cprintf((binhub_tcolor_t){CLR_BRT_GREEN, CLR_NONE}, "${env:" #var "}"); \
            printf(" = "); \
            binhub_cprintf((binhub_tcolor_t){CLR_CYAN, CLR_NONE}, "%s\n", var); \
        } \
    } while(0)
#else
    #define BINHUB_PRINT_ENV_VAR(var) do { \
        ...
        if(var == (void*)0) { \
            ...
            binhub_cprintf((binhub_tcolor_t){CLR_BRT_GREEN, CLR_NONE}, "${" #var "}"); \
            printf(" = "); \
            binhub_cprintf((binhub_tcolor_t){CLR_CYAN, CLR_NONE}, "%s\n", var); \
        } \
    } while(0)

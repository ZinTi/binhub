...
void MemoryHeader::insert(const std::string& header){
    if((header.front() == '<' && header.back() == '>') || (header.front() == '\"' && header.back() == '\"')){
        ...
        if(success){
            binhub_cprintf(BINHUB_COLOR_INIT(CLR_GREEN, CLR_NONE), "ok"); BINHUB_ENDLINE(1);
        } else {
            return;
        }
    } else {
        binhub_cprintf(BINHUB_COLOR_INIT(CLR_RED, CLR_NONE), "Error: Invalid library name format."); BINHUB_ENDLINE(1);
        return;
    }

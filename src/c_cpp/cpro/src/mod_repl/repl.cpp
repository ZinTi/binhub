...
int handle_cmd_repl(const int argc, char* argv[]){
	...
	for (unsigned int cyclecounter = 0;; cyclecounter++){
		/* 这部分：决定命令提示符的样式 */
		if (s > 0){
			binhub_cprintf(BINHUB_COLOR_INIT(CLR_BRT_CYAN, CLR_NONE), "C> ", 0);

			lines = 0;
			open_brace_count = close_brace_count = 0;
		}
		else if (s == -1 || s == -2 || s == -3){
			binhub_cprintf(BINHUB_COLOR_INIT(CLR_BRT_CYAN, CLR_NONE), "%d| ", lines);
		...
		if (judgRet1 == true){
			...
			if (judgRet2 == 1){
				...
				if (s == -1){ // 花括号未闭合
					...
					strcat(f_s_u, command_1);
					binhub_cprintf(BINHUB_COLOR_INIT(CLR_BRT_CYAN, CLR_NONE), "%s\n", f_s_u);
					...
					binhub_cprintf(BINHUB_COLOR_INIT(CLR_BRT_RED, CLR_NONE), "错误！出现落单的“}”"); BINHUB_ENDLINE(1);
				}
				else {
					// 预留
				}
...
int determine_type(const char* str){
	if (){ // 匹配 #include 预处理命令
		binhub_cprintf(BINHUB_COLOR_INIT(CLR_BRT_BLUE, CLR_NONE), "Lib: %s", str);
		putchar('\n');
		return 1;
	} else if (is_match(str, R"(^\s*#\s*define\s+\w+(\s*\([^)]*\))?\s+.+$)")){ // 匹配 #define 宏定义
		binhub_cprintf(BINHUB_COLOR_INIT(CLR_BRT_MAGENTA, CLR_NONE), "Macro: %s", str);
		putchar('\n');
		return 2;
	} else if (is_match(str, R"(^\s*\w+(\s+\w+)?\s+\w+\s*\([^)]*\)\s*\{[^}]*\}$)")){ // 匹配 C语言函数的定义
		binhub_cprintf(BINHUB_COLOR_INIT(CLR_BRT_CYAN, CLR_NONE), "Func: %s", str);
		putchar('\n');
		return 3;
	} else if (is_match(str, R"(^\s*struct\s+\w+\s*\{[^}]*\}\s*;$)")){ // 匹配 C语言结构体的定义
		binhub_cprintf(BINHUB_COLOR_INIT(CLR_BRT_YELLOW, CLR_NONE), "Struct: %s", str);
		putchar('\n');
		return 4;
	} else if (is_match(str, R"(^\s*union\s+\w+\s*\{[^}]*\}\s*;$)")){ // 匹配 C语言共用体的定义
		binhub_cprintf(BINHUB_COLOR_INIT(CLR_BRT_YELLOW, CLR_NONE), "Union: %s", str);
		putchar('\n');
		return 5;

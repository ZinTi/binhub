...
int MemoryProg::repl(int argc, char* argv[]){
    ...
    do{
        binhub_cprintf(BINHUB_COLOR_INIT(CLR_BRT_CYAN, CLR_NONE), "%s", this->prompt_.c_str()); // 打印提示符
        std::getline(std::cin, input); // 读取整行到字符串
        trim(input);    // 去除前导空格和尾随空格

        if(is_match(input.c_str(), R"(^\.(exit|quit)$)")){ // .exit, .quit
            binhub_cprintf(BINHUB_COLOR_INIT(CLR_BRT_CYAN, CLR_NONE), "Bye!"); BINHUB_ENDLINE(1);
            return 0;
            ...
            binhub_cprintf(BINHUB_COLOR_INIT(CLR_RED, CLR_NONE), "Invalid command!"); BINHUB_ENDLINE(1);
        }
...
int MemoryProg::handle_include_header(const std::string& full){
    ...
    for (const std::string& token : tokens) {
        ...
        if(token == ".inc"){
            ...
            binhub_cprintf(BINHUB_COLOR_INIT(CLR_RED, CLR_NONE), "Invalid arguments: %s", token.c_str()); BINHUB_ENDLINE(1);
        }
    }

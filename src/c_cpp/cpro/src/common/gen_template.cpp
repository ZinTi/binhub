...
int write_template_src(const std::string filename){
	...
	if (!strcmp(".c", strrchr(filename.c_str(), '.'))){
		...
		binhub_cprintf(BINHUB_COLOR_INIT(CLR_BRT_RED, CLR_NONE), "The file name extension cannot determine whether it is C or C++.");
		BINHUB_ENDLINE(1);
		exit(0);
	}
...
void show_version(void){
	...
	cout << "Copyright (c) 2026 Your Name. All rights reserved. your@email.com\n" << endl;
}
